...
There are `r scales::comma(nrow(oa_pop))` OAs in England and Wales. OAs contain on average `r scales::comma(median(oa_pop$`All Ages`))` people, although this ranges from `r scales::comma(min(oa_pop$`All Ages`))` up to `r scales::comma(max(oa_pop$`All Ages`))` people.
...
OAs vary significantly in geographical size, the smallest being `r subset(oa_bounds, Shape__Area == min(oa_bounds$Shape__Area))$LSOA11NM` (`r scales::comma(subset(oa_bounds, Shape__Area == min(oa_bounds$Shape__Area))$Shape__Area / 1000)`km^2) and the largest being `r subset(oa_bounds, Shape__Area == max(oa_bounds$Shape__Area))$LSOA11NM` (`r scales::comma(subset(oa_bounds, Shape__Area == max(oa_bounds$Shape__Area))$Shape__Area / 1000)`km^2).
...
There are `r scales::comma(nrow(msoa_pop))` MSOAs in England and Wales. MSOAs contain on average `r scales::comma(median(msoa_pop$`All Ages`))` people, although this ranges from `r scales::comma(min(msoa_pop$`All Ages`))` up to `r scales::comma(max(msoa_pop$`All Ages`))` people.
